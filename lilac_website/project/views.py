...
def add_homeproject(sender, instance, **kwargs):
    ...
    '''
    flag = False
    if instance.is_homepage is True:
...
class project_view(View):
    ...
    def show_detail(self, request, id):
        ...
        if (project.publisher == request.user) or (
                request.user.is_superuser is True):
            # 如果是发布者或者超级管理员进入对应页面
            return render(request, 'project/project_detail_self.html',
                          {'project': project})
        else:
            # 进入普通页面
            # 浏览量+1
            ...
            return render(request, 'project/project_detail_other.html',
                          {'project': project})
    ...
    def project_create(self, request):
        ...
        if request.user.is_authenticated is False:
            ...
            # 验证是否有发布权限
            if (request.user.is_superuser is False) and (profile.role == "ST"):
                # 如果不是超级管理员且是学生
            ...
            if project_form.is_valid():
                ...
                for i in range(len(all_applicants)):
                    if all_applicants[i] == " ":
                        ...
                        add_worker = User.objects.get_by_natural_key(
                            applicant_name)
                ...
                for i in range(len(applicants_delete)):
                    if applicants_delete[i] == " ":
                        ...
                        delete_applicant = User.objects.get_by_natural_key(
                            applicant_name)
                ...
                for i in range(len(workers_delete)):
                    if workers_delete[i] == " ":
                        ...
                        delete_worker = User.objects.get_by_natural_key(
                            delete_worker_name)
